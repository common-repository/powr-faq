<?php
     ...
     */
    /*
    Plugin Name: POWr FAQ
    Plugin URI: https://www.powr.io/plugins/faq
    Description: Answer questions in advance with custom FAQs  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.6.4
    ...
    */
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////
    ///////////////////////////////////////Create FAQ widget/////////////////////////////////
    class Powr_Faq extends WP_Widget{
      public $plugin;
      //Create the widget
      public function __construct(){
        parent::__construct( 'powr_faq',
                             __( 'POWr FAQ' ),
                             array( 'description' => __( 'FAQ by POWr.io') )
        );
      ...
      public function widget( $args, $instance ){
        ...
        ?>
        <div class='widget powr-faq' label='<?php echo $label; ?>'></div>
        <?php
      }
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your FAQ.
        </p>
        <p>
    ...
    function register_powr_faq() {
      register_widget( 'Powr_Faq' );
    }
    //Use widgets_init action hook to execute custom function
    add_action( 'widgets_init', 'register_powr_faq' );
    //Create short codes for adding plugins anywhere:
    function powr_faq_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-faq' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-faq' label='$label'></div>";
      }else{
      	"<div class='powr-faq'></div>";
      }
    }
    add_shortcode( 'powr-faq', 'powr_faq_shortcode' );
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_faq_menu' );
    function powr_faq_menu() {
      add_menu_page( 'POWr FAQ', 'POWr FAQ', 'manage_options', 'powr-faq-settings', 'powr_faq_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_faq_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/faq';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/faq';
      }
    ...
    register_activation_hook(__FILE__, 'powr_faq_plugin_activate');
      add_action('admin_init', 'powr_faq_plugin_redirect');

      function powr_faq_plugin_activate() {
      add_option('powr_faq_plugin_do_activation_redirect', true);
      }
      ...
      function powr_faq_plugin_redirect() {
      if (get_option('powr_faq_plugin_do_activation_redirect', false)) {
          delete_option('powr_faq_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-faq-settings&' );
          }
       }
      }
